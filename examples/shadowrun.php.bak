...
$attackerExpr = $phpdice->parse('10d6 >=5');
$attackerResult = $phpdice->roll($attackerExpr);
echo "   Rolled: " . implode(', ', $attackerResult->diceValues) . "\n";
echo "   Successes: {$attackerResult->successCount}\n\n";

echo "   Defender (8 dice):\n";
$defenderExpr = $phpdice->parse('8d6 >=5');
$defenderResult = $phpdice->roll($defenderExpr);
...
for ($i = 1; $i <= 3; $i++) {
    echo "   Attack {$i}:\n";
    $attackExpr = $phpdice->parse('12d6 >=5');
    $attackResult = $phpdice->roll($attackExpr);

    $defenseExpr = $phpdice->parse('10d6 >=5');
    $defenseResult = $phpdice->roll($defenseExpr);

    $netHits = $attackResult->successCount - $defenseResult->successCount;
