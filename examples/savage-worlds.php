...
$traitExpr = $phpdice->parse('1d8 explode');
$traitResult = $phpdice->roll($traitExpr);

$wildExpr = $phpdice->parse('1d6 explode');
$wildResult = $phpdice->roll($wildExpr);
...
$vigorExpr = $phpdice->parse('1d6 explode');
$vigorResult = $phpdice->roll($vigorExpr);
...
$shootingExpr = $phpdice->parse('1d8 explode - 2');
$shootingResult = $phpdice->roll($shootingExpr);

$wildExpr = $phpdice->parse('1d6 explode - 2');
$wildResult = $phpdice->roll($wildExpr);
...
while ($round <= 5 && $successes < 5) {
    echo "   Round {$round}: ";
    $skillExpr = $phpdice->parse('1d8 explode');
    $skillResult = $phpdice->roll($skillExpr);
...
echo "   First roll:\n";
$firstExpr = $phpdice->parse('1d6 explode + 1d6 explode');
...
if ($firstResult->total < 4) {
    ...
    $rerollExpr = $phpdice->parse('1d6 explode + 1d6 explode');
    $rerollResult = $phpdice->roll($rerollExpr);
...
for ($round = 1; $round <= 3; $round++) {
    ...
    $driver1Expr = $phpdice->parse('1d8 explode + 1d6 explode');
    $driver1Result = $phpdice->roll($driver1Expr);

    $driver2Expr = $phpdice->parse('1d6 explode + 1d6 explode');
    $driver2Result = $phpdice->roll($driver2Expr);
...
$spellExpr = $phpdice->parse('1d10 explode');
$spellResult = $phpdice->roll($spellExpr);
...
if ($castRoll >= 4) {
    ...
    $damageExpr = $phpdice->parse('2d6 explode' . ($raises > 0 ? " + {$raises}d6 explode" : ""));
    $damageResult = $phpdice->roll($damageExpr);
